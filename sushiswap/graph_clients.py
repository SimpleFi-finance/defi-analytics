    ...
    def getTransactionsOfClosedPositions(self, marketId):
        ...
        while True:
            ...
            for position in response['positions']:
                raw_positions[position['accountAddress']] = []
    ...
    def getFarmTransactionsForPositions(self, farm_address, farm_transactions, pool_transactions):
        ...
        for position_id in pool_transactions.keys():
            ...
            for tx in pool_transactions[position_id]:
                if tx["transactionType"] == "TRANSFER_OUT" and tx["transferredTo"] == farm_address:
                    farm_transactions_for_account = list(filter(lambda x: x["blockNumber"] >= tx["blockNumber"], farm_transactions_for_account))

                if tx["transactionType"] == "TRANSFER_IN" and tx["transferredFrom"] == farm_address:
                    transactions = list(filter(lambda x: x["blockNumber"] <= tx["blockNumber"], farm_transactions_for_account))
                    if not position_id in farm_transactions_for_positions: farm_transactions_for_positions[position_id] = []
                    farm_transactions_for_positions[position_id].extend(transactions)
                    farm_transactions_for_account = list(filter(lambda x: x["blockNumber"] > tx["blockNumber"], farm_transactions_for_account))
